...
import           System.IO
import           Text.Regex -- $ cabal install regex-compatghc
...
censor f writerT = WriterT $ fmap (\(a, w) -> (a, f w)) (runWriterT writerT)
...
--run the func writer, get the function in it,
    (<*>) :: (Monoid w, Applicative m) => WriterT w m (a -> b) -> WriterT w m a -> WriterT w m b

    writerTFunc <*> writerTArg = WriterT $ liftA2 combine (runWriterT writerTFunc) (runWriterT writerTArg) where
      combine (f, writ1) (a, writ2) = (f a, writ1 `mappend` writ2)


instance (Monoid w, Monad m) => Monad (WriterT w m) where
    (>>=) :: (Monoid w, Monad m) => WriterT w m a -> (a -> WriterT w m b) -> WriterT w m b
    writerT >>= f = WriterT $ do
      (result, written) <- runWriterT writerT
      (result', written') <- runWriterT $ f result
      return (result', written `mappend` written')
...
main = Socket.withSocketsDo $ do -- withSocketsDo is supposedly not needed anymore on most systems.
    ...
    -- Now we have to set up data structure explaining to the operating system explaining
